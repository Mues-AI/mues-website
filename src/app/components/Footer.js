...
export default function Footer({ variant = "light" }) {
  ...
  const FooterColumn = ({ categoryData }) => (
    <div className="flex flex-col gap-4">
      <p
        className={`${theme.categoryTitle} text-[14px] font-normal leading-[16px] tracking-[0.14px]`}
      >
        {categoryData.title}
      </p>
  ...
  return (
    <>
      <div className="flex flex-col mb-8">
        {/* FOOTER */}
        <div
          className={`w-full max-w-[1168px] h-auto flex flex-col mx-auto ${theme.background} rounded-xl p-6 md:p-9 lg:p-12 mt-4 md:mt-6`}
        >
          {/* 1. Line */}
          <div className="w-full flex flex-col gap-y-12 lg:gap-y-0 lg:flex-row lg:justify-between lg:gap-x-[clamp(16px,5vw,76px)]">
            <div className="w-fit h-fit flex flex-col gap-y-8 justify-between items-start">
              ...
              <div className="md:w-[320px]">
                <p className="w-fit md:w-[300px] h-auto tracking-[0.1px] text-[#AAA] font-urbanist text-[13px] font-medium leading-[20px]">
                  The Software Interaction Company
                </p>
                <p className="w-fit md:w-[300px] h-auto tracking-[0.1px] text-[#B6B6B6] text-[12px] font-light leading-[20px]">
                  to create a world where software adapts to humans, not humans
                  to software.
                </p>
              </div>
            </div>
              ...
              <div className="flex gap-x-2 xs:gap-x-3 md:gap-x-4">
                ...
                >
                  <XIcon
                    width={24}
                    height={24}
                    className={`${theme.socialIcon.base} ${theme.socialIcon.hover}`}
                  />
                </a>
                <a
                  href="https://www.linkedin.com/company/muesai/"
                  target="_blank"
                  rel="noopener noreferrer"
